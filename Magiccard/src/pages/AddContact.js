...
const AddContact = () => {
  ...
  //when u click update icon that particular document will be added in contactToUpdate..so it will not be null
  //by this we can ensure which ui we need to show i.e either update/add
  ...
  const imagePicker = async e => {
    ...
    try{
      ...
      var metadata={
        contentType:file.type //png or svg or jpeg
      }
      let resizedImage=await readAndCompressImage(file,imageConfig)
      const storageRef=await firebase.storage().ref()
      var uploadTask=storageRef
      .child('images/'+file.name)  //u can also give a randomly produced string
      .put(resizedImage,metadata)

      uploadTask.on(
        firebase.storage.TaskEvent.STATE_CHANGED,  //if anything is changed in the storage of firebase it throws a snapshot which we need to grab
        snapshot=>{  //this is a callback
          setIsUploading(true)
          ...
          {
            setIsUploading(false)
            toast("uploading",{type:"success"})

          }
        },
        error=>{
          toast("Something is wrong in state change",{type:"error"})
        }, //callback that u can do whatever u want
        ()=>{
      ...
      console.error(error);
      toast("something went wrong" ,{typpe:"error"})
    }
  };
        ...
        <Col md="6" className="offset-md-3 p-2">
          <Form onSubmit={handleSubmit}>
            <div className="text-center">
              {isUploading ? ( <Spinner type="grow" color="primary" />) : 
              (
                <div>
                  ...
                  <input type="file" name="image" id="imagepicker" accept="image/*" multiple={false}
                    onChange={e => imagePicker(e)} className="hidden"/>
                </div>
              )}
            </div>
            <FormGroup>
            ...
            <FormGroup>
              <Input type="number" name="number" id="phonenumber" value={phoneNumber} placeholder="phone number"
                onChange={e => setPhoneNumber(e.target.value)}/>
            </FormGroup>
            <FormGroup>
              <Input type="textarea" name="area" id="area" value={address} placeholder="address"
